...
import { Trash2, TrendingUp, TrendingDown, Building2, Loader2 } from "lucide-react"
                ...
                <TableRow key={holding.id} className="transition-colors duration-200 hover:bg-muted/50">
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      {isTSX && (
                        <Badge variant="outline" className="text-xs px-1 py-0 h-4 bg-red-500/10 text-red-400 border-red-500/20">
                          TSX
                        </Badge>
                      )}
                    </div>
                    {priceData && (
                      <span className="text-xs text-muted-foreground">{priceData.name}</span>
                    )}
                  </TableCell>
                  <TableCell>
                    <Badge
                  ...
                  <TableCell className="text-right tabular-nums">
                    {priceData ? (
                      <div className={`flex items-center justify-end gap-1 ${isUp ? "text-emerald-400" : isDown ? "text-red-400" : "text-muted-foreground"}`}>
                  ...
                  <TableCell className="text-right tabular-nums">
                    {value ? (
                      <span className="font-medium">
                        {priceData?.currency === "CAD" ? "C" : ""}${value.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    ) : (
                      <span className="text-muted-foreground">-</span>
                    )}
